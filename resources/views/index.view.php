<main class="bg-gray-200 p-4 text-gray-800 h-screen w-full">
    ...
    <form class="bg-gray-100 rounded p-4 container mx-auto" method="POST" action="/add-user">
        <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars(generateCsrfToken()) ?>">
        <h2 class="text-2xl font-bold mb-4">Add New User</h2>
        <div class="mb-4 flex gap-4 justify-center">
            <input type="text" name="name" required placeholder="Enter user name" class="px-2 py-1 rounded w-1/2 bg-gray-200 text-gray-800">
            <p><?= isset(\Deli\App\Forms\AddUserForm::$errors['name']) ? '<span class="text-red-600">'.htmlspecialchars(\Deli\App\Forms\AddUserForm::$errors['name']).'</span>' : '' ?></p>
            <input type="email" name="email" required placeholder="Enter user email" class="px-2 py-1 rounded w-1/2 bg-gray-200 text-gray-800">
            <p><?= isset(\Deli\App\Forms\AddUserForm::$errors['email']) ? '<span class="text-red-600">'.htmlspecialchars(\Deli\App\Forms\AddUserForm::$errors['email']).'</span>' : '' ?></p>
        </div>
